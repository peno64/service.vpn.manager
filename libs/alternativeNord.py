...
def getNordVPNLocation(vpn_provider, location, server_count, just_name):
    ...
    try:
      f = open('/storage/downloads/discardVPNservers.txt')
      discards = set([line.rstrip('\n').rstrip('\r') for line in f])
      f.close()
    except Exception as e:
      discards = set()
    ...
    try:
      f = open('/storage/downloads/setVPNserver.txt')
      servers = f.readlines()
      f.close()
    except Exception as e:
      servers = []
    ...
    #xbmcgui.Dialog().ok(xbmcaddon.Addon().getAddonInfo('name'), "server: " + server)
    #server = "be84.nordvpn.com"
